...
void lex(const char* src) {
    ...
    while (*p) {
        ...
        if (strncmp(p, "integer", 7) == 0 && isspace(p[7])) {
            ...
            p++; // skip jika kosong/unknown instruc
        }
    }
...
Node* parse_echo() {
    ...
    if (!match(TOKEN_LPAREN)) {
        printf("\033[1;31mExpected '(' after echo\033[0m\n");
        return NULL;
    }
    ...
    if (!match(TOKEN_RPAREN)) {
        printf("\033[1;31mExpected ')' after echo string\033[0m\n");
        return NULL;
    }
...
Node* parse_if() {
    consume(); //Jembud kuda
    if (!match(TOKEN_LPAREN)) {
        printf("\033[1;31mExpected '(' after if\033[0m\n");
        return NULL;
    }
    ...
    ifnode->type = NODE_IF;
    ifnode->condition = make_var(var_token->text);
    ifnode->expr = make_literal(atoi(val_token->text));
...
Node* parse_program() {
    ...
    while (peek()->type != TOKEN_EOF) {
        ...
        else if (peek()->type == TOKEN_IDENTIFIER && strcmp(peek()->text, "echo") == 0) {
            stmt = parse_echo();
        }
...
void generate_c_code(Node *node, FILE *output, int indent);

void generate_assignment(Node* node, FILE* output, int indent) {
    for (int i = 0; i < indent; i++) fprintf(output, "    ");
    fprintf(output, "int %s = %d;\n", node->var_name, node->expr->literal_value);
}

void generate_echo(Node* node, FILE* output, int indent) {
    for (int i = 0; i < indent; i++) fprintf(output, "    ");
    fprintf(output, "printf(\"%s\\n\");\n", node->var_name);
}

void generate_if(Node* node, FILE* output, int indent) {
    for (int i = 0; i < indent; i++) fprintf(output, "    ");
    fprintf(output, "if (%s == %d) {\n", node->condition->var_name, node->expr->literal_value);
    
    Node* then_node = node->then_branch;
    while (then_node) {
        generate_c_code(then_node, output, indent + 1);
...
void generate_c_code(Node* node, FILE* output, int indent) {
    while (node) {
        switch (node->type) {
            case NODE_ASSIGN:
                generate_assignment(node, output, indent);
                break;
            case NODE_IF:
                generate_if(node, output, indent);
                break;
            case NODE_ECHO:
                generate_echo(node, output, indent);
                break;
            default:
...
int main(int argc, char **argv) {
    ...
    printf("\033[1;32mCompiler C Backend\033[0m\n");
    printf("\033[1;32mLexer & Parser Code\033[0m\n");
    ...
    generate_c_code(ast, output, 1);
    fprintf(output, "    return 0;\n");
    fprintf(output, "}\n");
    fclose(output);

    printf("\nGenerated C code saved to 'output.c'.\n");
